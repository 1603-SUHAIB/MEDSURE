...
# Generate a key for encryption/decryption and save it securely
...
def manufacturer():
    if request.method == 'POST':
        ...
        return render_template('manudash.html')  # Or redirect to the dashboard
...
def pharma():
    if request.method == 'POST':
        # Handle login logic here (e.g., validate email and password)
        return render_template('pharma.html')  # Or redirect to the pharmacy dashboard
...
def generate():
    ...
    encrypted_data = cipher.encrypt(verification_url.encode())
...
def verify(unique_id):
    if request.method == 'POST':
        otp_input = request.form['otp']  # Now this will be the single OTP input field

        if unique_id in data_store:
            data = data_store[unique_id]
            print(f"Received OTP input: {otp_input}")  # Debugging statement
            print(f"Stored OTP: {data['otp']}")  # Debugging statement

            if otp_input == data['otp']:
                # Mark the data as verified and remove it from the data_store
                data_store[unique_id]['verified'] = True  # Update the verified status
...
def read_qr_code_from_image(image_path):
    ...
    for qr_code in qr_codes:
        encrypted_data = qr_code.data.decode("utf-8")
        try:
...
def upload():
    ...
    if file:
        ...
        if qr_data and is_url(qr_data):
            ...
            return "No valid or decryptable QR code detected!"
...
def scan():
    ...
    while True:
        ...
        for qr_code in qr_codes:
            encrypted_data = qr_code.data.decode("utf-8")
            try:
                qr_data = cipher.decrypt(encrypted_data.encode()).decode()
                break
            except Exception as e:
                print(f"Decryption failed: {e}")
                continue
